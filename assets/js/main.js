...
function saveContent() {
    downloadFile("index.html", document.getElementById("html-code").value, "text/html");
}

// Function to save CSS content to a file
function saveCSSContent() {
    downloadFile("style.css", document.getElementById("css-code").value, "text/css");
}

// Function to save JS content to a file
function saveJSContent() {
    downloadFile("script.js", document.getElementById("js-code").value, "text/javascript");
}
...
// Function to handle Ctrl+A, Ctrl+C, Ctrl+X, and Tab within textareas
function handleTextareaKeyEvents(textareaId, event) {
    ...
    if (event.ctrlKey) {
        ...
        textarea.value = currentContent.substring(0, start) + tabCharacter + currentContent.substring(end);
...
textAreaIds.forEach(textareaId => {
    const textarea = document.getElementById(textareaId);
    textarea.addEventListener("keydown", event => handleTextareaKeyEvents(textareaId, event));
});